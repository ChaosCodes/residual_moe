...
def z_loss_func(logits: torch.Tensor) -> float:
    ...
    return torch.mean(torch.square(torch.logsumexp(concatenated_gate_logits, dim=-1)))
...
class MixtralForCausalLM(MixtralPreTrainedModel):
    ...
    def forward(
        ...
        if output_router_logits:
            z_loss = z_loss_func(outputs.router_logits if return_dict else outputs[-1])
            ...
            if labels is not None:
                loss += self.router_aux_loss_coef * aux_loss.to(loss.device) + 0.001 * z_loss.to(loss.device) # make sure to reside in the same device
